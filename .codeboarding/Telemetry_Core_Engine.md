```mermaid
graph LR
    Telemetry_Orchestrator["Telemetry Orchestrator"]
    Identity_Resolver["Identity Resolver"]
    Event_Dispatcher["Event Dispatcher"]
    Telemetry_Schema_Registry["Telemetry Schema Registry"]
    Event_Dispatcher -- "passes enriched events to" --> Telemetry_Orchestrator
    Event_Dispatcher -- "validates events against" --> Telemetry_Schema_Registry
    Telemetry_Orchestrator -- "requests unique identifier from" --> Identity_Resolver
    Identity_Resolver -- "provides identity metadata to" --> Telemetry_Schema_Registry
    Event_Dispatcher -- "calls" --> Identity_Resolver
```
