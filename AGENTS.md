...
- **Avoid nested/anonymous functions that grow their enclosing function**: don't define inner `def`s or non-trivial `lambda`s that push a function past a screenful or capture enclosing state to do real work. Extract them to module-level (or a method on the relevant type) so the parent stays readable and the helper is testable and reusable. A one-line `lambda` as a `key=`/`sorted()` argument is fine; a multi-branch closure that another block depends on is not.
