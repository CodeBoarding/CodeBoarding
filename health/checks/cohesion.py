...
def check_component_cohesion(call_graph: CallGraph, config: HealthCheckConfig) -> StandardCheckSummary:
    ...
    NOT wired into ``health.runner`` today: ``cluster_call_graph`` seeds the
    graph's cluster cache with a current-graph partition, which would
    masquerade as a historical snapshot in the next incremental delta. Before
    re-enabling, either reset the cache after the call or compute the
    partition without touching it.
    """
    warning_entities: list[FindingEntity] = []

    cluster_result = cluster_call_graph(call_graph)
