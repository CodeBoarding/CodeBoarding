...
class LSPDiagnosticsCollector:
    ...
    def add_diagnostic(self, file_path: str, diagnostic: dict[str, Any]) -> None:
        """Add a raw LSP diagnostic.

        Args:
            file_path: Path to the file where the diagnostic was reported
            diagnostic: Raw LSP diagnostic object
        """
        self.diagnostics.append({"file_path": file_path, "diagnostic": diagnostic})
    ...
    def process_diagnostics(self) -> list[DiagnosticIssue]:
        ...
        for item in self.diagnostics:
            ...
            issue = self._convert_to_issue(file_path, diagnostic)
            if issue:
    ...
    def _convert_to_issue(self, file_path: str, diagnostic: dict[str, Any]) -> DiagnosticIssue | None:
