...
def _visible_targets(graph: CallGraph, node: Node, skip_set: set[Node]) -> list[str]:
    ...
    Why: a name that resolves to nothing in the graph is an external call, not a
    filtered-out one, so it stays — only targets that *are* skipped nodes are dropped.
    """
    visible = []
