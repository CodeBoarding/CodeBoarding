...
def _format_member_delta(delta: ClusterMemberDelta) -> str:
    ...
    if delta.modified_methods:
        parts.append(f"modified={sorted(delta.modified_methods)}")
    if delta.edge_changes:
        parts.append(f"graph_changes={[change.llm_str() for change in delta.edge_changes]}")
