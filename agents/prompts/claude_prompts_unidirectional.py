...
Create architectural documentation and visual diagrams by analyzing control flows and component relationships. Prioritize efficiency, accuracy, single-pass analysis, and diagram generation suitability.
...
Apply {project_type} patterns. Focus on core business logic - exclude logging/error handling components that clutter diagrams.

Use getPackageDependencies or getClassHierarchy only when relationships need clarification for diagram connections."""
...
Use getClassHierarchy for component structure clarification. Use readFile for boundary validation.

Work with existing insights. Focus on flow graph representation."""
...
For call/return relationships, keep only the call relationship to maintain diagram clarity and direction.

Components should have distinct visual boundaries and clear naming. Exclude utility/logging components that clutter diagrams."""
...
Address feedback systematically while maintaining analysis integrity. Focus only on changes that improve documentation clarity and flow diagram representation.

Use tools to address missing information affecting diagram generation suitability."""
