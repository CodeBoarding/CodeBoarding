...
2. For modified clusters, preserve the existing owning component shown by its clusters=[...] list; use update_component for that owner instead of moving the cluster to another component. A cluster id already listed in another component's clusters=[...] is already owned; do not assign it to a different component or a new component.
3. For new clusters, first check routing facts and structural diff method status: code_added methods can justify create_component; existing_in_base methods usually mean scope/cluster movement and should update or stay with the existing owner. Do not infer this from file/package layout alone.
