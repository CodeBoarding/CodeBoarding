...
def _build_files_index_from_analysis(analysis: AnalysisInsights) -> dict[str, FileEntry]:
    ...
    for file_path in sorted(analysis.files):
        entry = analysis.files[file_path].model_copy(deep=True)
        entry.methods = _sort_method_entries(entry.methods)
...
def _sort_method_entries(methods: list[MethodEntry]) -> list[MethodEntry]:
    return sorted(
        methods,
        key=lambda method: (
            method.start_line,
...
def _sort_key_entities(entities: list[SourceCodeReference]) -> list[SourceCodeReference]:
    return sorted(
        entities,
        key=lambda entity: (
            entity.qualified_name,
            entity.reference_file or "",
            entity.reference_start_line or 0,
            entity.reference_end_line or 0,
        ),
    )


def _sort_component_file_groups(groups: list["ComponentFileMethodGroupJson"]) -> list["ComponentFileMethodGroupJson"]:
    return sorted(groups, key=lambda group: (group.file_path, tuple(group.methods), group.file_status))


def _sort_relations(relations: list["RelationJson"]) -> list["RelationJson"]:
    return sorted(
        relations,
        key=lambda relation: (
            relation.src_id,
...
def _sort_components(components: list["ComponentJson"]) -> list["ComponentJson"]:
    return sorted(components, key=lambda component: (component.component_id, component.name))
...
def _to_component_file_method_refs(file_methods: list[FileMethodGroup]) -> list["ComponentFileMethodGroupJson"]:
    ...
    for group in sorted(file_methods, key=lambda item: item.file_path):
        qnames: list[str] = []
        seen: set[str] = set()
        for method in _sort_method_entries(group.methods):
    ...
    return _sort_component_file_groups(refs)
...
def _build_methods_index_from_files(files_index: dict[str, FileEntry]) -> dict[str, "MethodIndexEntry"]:
    ...
    for file_path in sorted(files_index):
        entry = files_index[file_path]
        for method in _sort_method_entries(entry.methods):
...
def from_component_to_json_component(
    ...
    if can_expand and sub_analyses and component.component_id in sub_analyses:
        ...
        nested_components = _sort_components(
            [
                from_component_to_json_component(c, sub_expandable, sub_analyses, processed_ids)
                for c in sub_analysis.components
            ]
        )
        nested_relations = _sort_relations([_relation_to_json(r) for r in sub_analysis.components_relations])

    return ComponentJson(
        name=component.name,
        component_id=component.component_id,
        description=component.description,
        key_entities=_sort_key_entities(component.key_entities),
        source_cluster_ids=component.source_cluster_ids,
        file_methods=_sort_component_file_groups(_to_component_file_method_refs(component.file_methods)),
...
def from_analysis_to_json(
    ...
    components_json = _sort_components(
    ...
    relations_json = _sort_relations([_relation_to_json(r) for r in analysis.components_relations])
