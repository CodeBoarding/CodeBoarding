...
class DiagramGenerator:
    ...
    def _record_incremental_cluster_paths(self, cluster_results: dict[str, ClusterResult]) -> None:
        """Record root cluster paths after an incremental Infomap update."""
        if self.static_analysis is None:
            return
        for language, cr in cluster_results.items():
            try:
                cfg = self.static_analysis.get_program_graph(Language(language))
    ...
    def rebuild_global_relations(
        ...
        *,
        ...
        previous_relations: list | None = None,
    ) -> list:
        ...
        if preserve_unchanged:
            previous = {
                _relation_topology_key(relation): relation
                for relation in (
                    previous_relations if previous_relations is not None else root_analysis.components_relations
                )
            }
            global_relations = [
                previous.get(_relation_topology_key(relation), relation) for relation in global_relations
            ]
    ...
    def finalize_for_save(
        ...
        sub_analyses: dict[str, AnalysisInsights],
        *,
        preserve_unchanged_relations: bool = False,
        previous_relations: list | None = None,
    ) -> None:
        ...
        """
        self.rebuild_global_relations(
            root_analysis,
            sub_analyses,
            preserve_unchanged=preserve_unchanged_relations,
    ...
    def finalize_and_save(
        ...
        *,
        ...
        """
        self.finalize_for_save(
            root_analysis,
            sub_analyses,
            preserve_unchanged_relations=preserve_unchanged_relations,
            previous_relations=previous_relations,
        )
        ...
        if seed_delta is not None:
            self._record_incremental_cluster_paths(seed_delta)
        if persist_side_artifacts:
    ...
    def generate_analysis_incremental(
        ...
        monitor = self.stats_writer if self.stats_writer else nullcontext()
        with monitor:
            baseline_relations = list(root_analysis.components_relations)
            for analysis in (root_analysis, *sub_analyses.values()):
                refresh_method_locations_from_cfg(analysis, self.static_analysis, self.repo_location)
            ...
            if not old_snapshot.all_cluster_ids():
                # No Infomap lineage on the live graph — no prior pkl, legacy pkl,
            ...
            delta = compute_cluster_delta(
                ...
                changes=self.changes,
                repo_dir=self.repo_location,
                old_static=snapshot_source,
                previous_files=root_analysis.files,
            )
            if not delta.has_changes:
                ...
                return self.finalize_and_save(
                    root_analysis,
                    sub_analyses,
                    preserve_unchanged_relations=True,
                    previous_relations=baseline_relations,
                )
            ...
            if apply_result.relation_contexts:
                ...
                _restore_untouched_relations(
                    root_analysis,
                    baseline_relations,
                    apply_result.refresh_ids | apply_result.new_component_ids | apply_result.removed_ids | removed_ids,
                )

            self._refresh_files_index(root_analysis, sub_analyses)

            analysis_path = self.finalize_and_save(
                root_analysis,
                sub_analyses,
                seed_delta=delta.cluster_results(),
                preserve_unchanged_relations=True,
                previous_relations=baseline_relations,
            )
...
def _relation_topology_key(relation) -> tuple:
    """Identity for freezing relations whose architecture did not change."""
    ...
    return (
        ...
        relation.evidence,
        relation.is_static,
        tuple(sorted(edge_key(edge) for edge in relation.key_edges)),
        tuple(sorted(edge_key(edge) for edge in relation.all_edges)),
    )


def _restore_untouched_relations(
    root_analysis: AnalysisInsights,
    baseline_relations: list,
    affected_component_ids: set[str],
) -> None:
    """Keep exact relation objects outside the changed component frontier."""

    def touches_affected_subtree(relation) -> bool:
        return any(
            is_self_or_descendant(endpoint_id, affected_id)
            for affected_id in affected_component_ids
            for endpoint_id in (relation.src_id, relation.dst_id)
        )

    untouched = [relation for relation in baseline_relations if not touches_affected_subtree(relation)]
    touched = [relation for relation in root_analysis.components_relations if touches_affected_subtree(relation)]
    root_analysis.components_relations = [*untouched, *touched]
