...
class InterestingModules(BaseModel):
    interesting_modules: list[str] = Field(
...
@tool("read_source_code", args_schema=ModuleInput, return_direct=True)
...
class AbstractionAgent:
    def __init__(self, project_name):
        ...
        # interesting_modules_parser = JsonOutputParser(pydantic_object=InterestingModules)
        self.cfg_prompt = PromptTemplate(template=CFG_PROMPT_TEXT,
                                         input_variables=["project_name", "cfg_str"],)
                                         # partial_variables={
                                         #     "format_instructions": interesting_modules_parser.get_format_instructions()})
    ...
    def get_interesting_modules(self, cfg_str, nodes):
        ...
        response = self.agent.invoke({ "messages": [SystemMessage(content=SYSTEM_MESSAGE), HumanMessage(content=ask_for_abstraction)]})
        return response
