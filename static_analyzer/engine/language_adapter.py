...
_GENERATED_MARKER_RE = re.compile(
    rb"@generated|auto[- ]?generated|do not edit|generated by|clang2py|ctypesgen|ctypeslib",
    re.IGNORECASE,
)


def _is_generated_source(path: Path) -> bool:
    """Heuristic: machine-generated source (protobuf, ctypes/clang2py bindings, ...).

    Why: such files carry enormous symbol counts but no architectural signal, and they make the
    LSP references phase (one query per symbol position) explode — e.g. tinygrad's
    ``runtime/autogen`` ctypes bindings are ~82% of its trackable positions. Detected by an
    explicit generated-header marker, or a large ctypes-binding module (clang2py/ctypesgen style).
    """
    try:
        ...
        return False
    if _GENERATED_MARKER_RE.search(head):
        return True
    return b"import ctypes" in head and size > 200_000
