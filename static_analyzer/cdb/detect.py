...
def locate_user_cdb(project_root: Path) -> Path | None:
    ...
    a malformed user CDB falls through to detection/generation rather
    than silently producing empty clangd analysis. ``compile_flags.txt``
    is accepted on existence (clangd reads it as raw flags).
    """
    for sub in _PROBE_SUBDIRS:
...
def install_hint_for(detection: DetectionResult | BuildSystemKind) -> str:
    ...
    # Why: dialect-neutral example. ``-std=c++20`` is wrong for pure-C repos
    # (clangd parses ``.c`` files with the C dialect by default per extension).
    return (
        "No recognised build system at the project root. The simplest fix is a "
        "compile_flags.txt with e.g. '-Iinclude\\n-Wall' -- clangd reads "
        "it verbatim as flags for every source file under the project and picks "
        "the dialect (C vs C++) per file extension."
    )
