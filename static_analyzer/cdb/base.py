...
class CdbGenerator(ABC):
    ...
    def _fingerprint_inputs(self, project_root: Path) -> list[Path]:
        ...
        marker (Makefile / CMakeLists.txt / BUILD) actually lives.
        """
...
def run_build_step(
    ...
    *,
    ...
    Why: every generator that shells out (Bear, CMake, Meson, Ninja, Bazel) wants the
    ...
    ``timeout`` defaults to ``config.generator_timeout_seconds()``. ``timeout_hint``
    is appended to the timeout message so generators with their own scoping knob
    (e.g. Bazel's query) can point users at it alongside ``ENV_TIMEOUT``.
    """
    ...
    try:
        ...
        message = (
            f"{step} timed out after {effective_timeout}s in {cwd}. Raise {config.ENV_TIMEOUT} to allow more time."
        )
        if timeout_hint:
