...
from static_analyzer.cdb.base import CDB_SKIP_DIRS
...
def compute_fingerprint(
    ...
    *,
    ...
    ``metadata`` is an optional list of ``(key, value)`` pairs hashed after
    the file content so generation knobs (build target, configure args,
    Bazel query scope, generator kind) participate in the cache key. Pairs
    are sorted before hashing so callers don't have to. ``metadata=None``
    is equivalent to the no-metadata digest produced before this parameter
    existed — backward-compatible for legacy callers and tests.
    """
    ...
    for path in sorted(unique.values(), key=lambda p: str(p)):
        ...
        hasher.update(b"\0\0")
    if metadata:
        for meta_key, meta_value in sorted(metadata):
            hasher.update(meta_key.encode("utf-8"))
            hasher.update(b"\0")
            hasher.update(meta_value.encode("utf-8"))
            hasher.update(b"\0\0")
