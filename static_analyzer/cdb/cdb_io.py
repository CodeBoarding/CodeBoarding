...
class EmptyCompilationDatabaseError(ValueError):
    """Raised when a CDB is structurally valid JSON but contains zero entries.

    Why: a generic "must contain a JSON array" error pointed users at the
    file shape when the real problem was an empty build graph (Bear
    captured nothing, ``ninja -t compdb`` printed ``[]``). Surface that
    explicitly so the diagnostic matches Bazel's "returned no CppCompile
    actions" message. ``ValueError`` parent keeps existing callers
    catching ``ValueError`` working unchanged.
    """
...
def validate_compile_commands(payload: object) -> list[dict]:
    ...
    if not payload:
        raise EmptyCompilationDatabaseError(
            "compile_commands.json is an empty array (no compile actions found). "
            "The build tool ran but emitted no C/C++ compile commands - check that "
            "the build actually compiles source files (e.g. 'make clean all' before "
            "'bear --', or widen CODEBOARDING_CPP_BAZEL_QUERY)."
        )
