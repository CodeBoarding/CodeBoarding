"""Leaf clustering of one CallGraph — the algorithm behind ``ClusterResult``.

Extracted from ``CallGraph`` so the graph stays a data object; the service and
helpers call ``cluster_call_graph`` instead of a graph method.
"""
...
def detect_communities[T](
    ...
    *,
    ...
    Wraps ``leidenalg.find_partition`` indirectly so callers don't import
    ``igraph``/``leidenalg`` themselves — the dependency surface is contained
    to ``leiden_utils``.
    """
    return _leiden_find_partition(graph, weight=weight, resolution=resolution, seed=seed)


def cluster_call_graph(
    cfg: CallGraph,
    ...
    """Cluster *cfg* using a try-all-then-level-up approach, caching on the graph.
    ...
    """
    if cfg.cluster_cache is not None:
        return cfg.cluster_cache

    nx_graph = cfg.to_networkx_with_references()
    if nx_graph.number_of_nodes() == 0:
        logger.warning("No nodes available for clustering.")
        cfg.set_cluster_cache(ClusterResult(strategy="empty"))
        assert cfg.cluster_cache is not None
        return cfg.cluster_cache
    ...
    for level in levels:
        if level is None:
            ...
            work_graph = _cluster_at_level(nx_graph, level)
        ...
        if level is not None:
            candidates = _map_candidates_to_original(candidates, nx_graph, level, min_cluster_size, total_nodes)
    ...
    if all_candidates:
        ...
        if best_score > 0.0:
            cfg.set_cluster_cache(_build_result(best_communities, best_strategy, min_cluster_size, nx_graph))
            assert cfg.cluster_cache is not None
            return cfg.cluster_cache
...
def _cluster_with_algorithm(graph: nx.DiGraph, algorithm: str) -> list[set[str]]:
    # Use the shared seed for reproducibility - Leiden/Louvain are non-deterministic without it
    if algorithm == "leiden":
...
def _get_abstract_node_name(node_name: str, level: str) -> str:
    """Abstract a qualified name to its class/file/package grouping."""
    delimiter = ClusteringConfig.QUALIFIED_NAME_DELIMITER
...
def _cluster_at_level(graph: nx.DiGraph, level: str) -> nx.DiGraph:
    """Create abstracted graph by grouping nodes at the given level."""
    abstracted = nx.DiGraph()
    node_map: dict[str, str] = {}

    for node in graph.nodes():
        abstract_name = _get_abstract_node_name(node, level)
...
def _try_all_algorithms(
    ...
    Returned as a list so ``cluster_call_graph``'s cross-level pooling stays uniform.
    """
...
def _map_candidates_to_original(
    ...
    for node in original_graph.nodes():
        abstract_to_original[_get_abstract_node_name(node, level)].append(node)
