...
graph LR
    ...
    Orchestration_Infrastructure -- "initializes LSP servers and manages repository lifecycle" --> Static_Analysis_Graph_Engine
    Orchestration_Infrastructure -- "configures user settings and monitors execution context" --> Knowledge_Telemetry
    Static_Analysis_Graph_Engine -- "persists call graphs and caches analysis results" --> Knowledge_Telemetry
    Static_Analysis_Graph_Engine -- "provides raw symbol data and static relations for mapping" --> Synthesis_Output
    AI_Reasoning_Health -- "queries call graphs and clusters for semantic reasoning" --> Static_Analysis_Graph_Engine
    AI_Reasoning_Health -- "generates architectural insights and component classifications" --> Synthesis_Output
    AI_Reasoning_Health -- "resolves command paths and manages remote repository clones" --> Orchestration_Infrastructure
    Synthesis_Output -- "accesses file system locks and repository ignore rules" --> Orchestration_Infrastructure
    Synthesis_Output -- "triggers agentic workflows for incremental analysis" --> AI_Reasoning_Health
    Synthesis_Output -- "archives final analysis artifacts and tracks generation metrics" --> Knowledge_Telemetry
...
graph LR
    ...
    Community_Detection_Graph_Analysis["Community Detection & Graph Analysis"]
    Analysis_State_Agent_Interface["Analysis State & Agent Interface"]
    LSP_Orchestration_Language_Adapters -- "provides raw symbol and reference data for graph construction" --> Semantic_Graph_Builder
    LSP_Orchestration_Language_Adapters -- "triggers result conversion and cache invalidation" --> Analysis_State_Agent_Interface
    Semantic_Graph_Builder -- "requests file-specific language IDs and document synchronization" --> LSP_Orchestration_Language_Adapters
    Community_Detection_Graph_Analysis -- "queries call graph structures for subgraph extraction" --> Analysis_State_Agent_Interface
    Analysis_State_Agent_Interface -- "inspects symbol metadata to filter local variables and references" --> Semantic_Graph_Builder
    Analysis_State_Agent_Interface -- "delegates clustering logic and cross-language budget enforcement" --> Community_Detection_Graph_Analysis
    Analysis_State_Agent_Interface -- "queries language-specific metadata during format conversion" --> LSP_Orchestration_Language_Adapters
...
Manages the lifecycle of language-specific servers and normalizes their outputs into a common protocol format.
...
Constructs the global call graph by populating a symbol table from LSP data and resolving cross-file references.
...
Applies clustering algorithms to the call graph to identify logical components and simplify architectural visualization.
...
### Analysis State & Agent Interface
Manages persistence of analysis results, handles incremental updates, and provides tools for AI agents to query the codebase.
...
- `agents.tools.read_cfg.GetCFGTool`:8-61
- `static_analyzer.analysis_cache.invalidate_files`:335-392
