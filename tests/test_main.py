...
class TestSetupEnvironment(unittest.TestCase):
    def setUp(self):
        ...
        env_vars = ["REPO_ROOT", "ROOT_RESULT", "STATIC_ANALYSIS_CONFIG", "PROJECT_ROOT", "DIAGRAM_DEPTH_LEVEL"]
        for var in env_vars:
            if var in os.environ:
                del os.environ[var]

    def test_setup_environment_defaults(self):
        # Test that default values are set when env vars are not set
        with tempfile.TemporaryDirectory() as temp_dir:
            project_root = Path(temp_dir)
            setup_environment(project_root)

            self.assertEqual(os.getenv("REPO_ROOT"), str(project_root / "repos"))
            self.assertEqual(os.getenv("ROOT_RESULT"), str(project_root / "results"))
            self.assertEqual(os.getenv("STATIC_ANALYSIS_CONFIG"), str(project_root / "static_analysis_config.yml"))
            self.assertEqual(os.getenv("PROJECT_ROOT"), str(project_root))
            self.assertEqual(os.getenv("DIAGRAM_DEPTH_LEVEL"), "1")

    def test_setup_environment_existing_values(self):
        # Test that existing env vars are not overwritten
        os.environ["REPO_ROOT"] = "/custom/repo"
        os.environ["ROOT_RESULT"] = "/custom/result"

        with tempfile.TemporaryDirectory() as temp_dir:
            project_root = Path(temp_dir)
            setup_environment(project_root)

            self.assertEqual(os.getenv("REPO_ROOT"), "/custom/repo")
            self.assertEqual(os.getenv("ROOT_RESULT"), "/custom/result")

    def test_setup_environment_none_project_root(self):
        # Test with None project_root (should use current directory)
        setup_environment(None)

        # Should set DIAGRAM_DEPTH_LEVEL at minimum
        self.assertEqual(os.getenv("DIAGRAM_DEPTH_LEVEL"), "1")
