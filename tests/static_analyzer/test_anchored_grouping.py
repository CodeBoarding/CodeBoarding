...
Fresh grouping re-optimizes from scratch, which is deterministic but
not continuous — modularity has many near-equal optima, so a small edit can select a
different one and reshuffle ownership. The incremental path uses anchored grouping
...
class TestAnchoredGrouping(unittest.TestCase):
    ...
    def _anchored(self, cluster_result, graph, previous_owner, *, subcomponents=False):
        return self.grouping_service.anchored_group(
            {"test": cluster_result},
            {"test": graph},
            previous_owner,
    ...
    def test_unchanged_input_keeps_every_owner(self):
        ...
        result = self._anchored(cr, graph, previous)
    ...
    def test_a_new_cluster_is_absorbed_and_moves_nothing_else(self):
        cr, graph = blocks(n_blocks=5, per_block=4)
        previous = self._previous(cr, graph)
        before = self._anchored(cr, graph, previous)
        ...
        after = self._anchored(cr, graph, previous)
    ...
    def test_a_removed_cluster_moves_nothing_else(self):
        cr, graph = blocks(n_blocks=5, per_block=4)
        previous = self._previous(cr, graph)
        before_owners = owners_by_member(self._anchored(cr, graph, previous), cr)
        ...
        after_owners = owners_by_member(self._anchored(cr, graph, previous), cr)
    ...
    def test_no_previous_grouping_falls_back_to_a_fresh_partition(self):
        ...
        result = self._anchored(cr, graph, {})
    ...
    def test_is_deterministic(self):
        ...
        first = self._anchored(cr, graph, previous)
        second = self._anchored(cr, graph, previous)
    ...
    def test_works_at_sub_component_range(self):
        ...
        result = self._anchored(cr, graph, previous, subcomponents=True)
    ...
    def test_reported_modularity_scores_the_returned_grouping(self):
        ...
        groups, modularity = self.grouping_service.group({"test": cr}, {"test": graph})
    ...
    def test_absorbing_an_edged_leftover_moves_the_score(self):
        ...
        seeds, leftovers = _seeds_from_partition(communities, method_count, DEFAULT_GROUPING_CONFIG)
