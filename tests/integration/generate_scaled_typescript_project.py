#!/usr/bin/env python3
"""Generate a scaled-up TypeScript edge cases project.

Duplicates the base typescript_edge_cases_project N times with unique directory
suffixes (models_00, models_01, ...), creates entry_XX.ts files (one per copy
of src/index.ts), and a mega src/index.ts that imports all entries.
Also generates the corresponding fixture JSON.

Usage:
    uv run python tests/integration/generate_scaled_typescript_project.py
    uv run python tests/integration/generate_scaled_typescript_project.py --copies 50
"""
...
SOURCE_PROJECT = SCRIPT_DIR / "projects" / "typescript_edge_cases_project"
TARGET_PROJECT = SCRIPT_DIR / "projects" / "typescript_scaled_project"
FIXTURE_DIR = SCRIPT_DIR / "fixtures"
SOURCE_FIXTURE = FIXTURE_DIR / "typescript_edge_cases.json"
...
def rewrite_imports(content: str, i: int) -> str:
    ...
    Within-directory imports (e.g., './base', './helpers') are left unchanged.
    Cross-directory imports (e.g., '../utils', './models') get the suffix.
    """
    s = sfx(i)
    for pkg in PACKAGES:
        # "../models" → "../models_00"  (from services/ or models/ referencing siblings)
        content = content.replace(f'"../{pkg}"', f'"../{pkg}{s}"')
        # "./models" → "./models_00"  (from src/index.ts referencing subdirs)
        content = content.replace(f'"./{pkg}"', f'"./{pkg}{s}"')
    return content


def rewrite_ts_name(name: str, i: int) -> str:
...
def rewrite_ts_pkg(pkg: str, i: int) -> str:
    """Rewrite a package name for copy i."""
    s = sfx(i)

    if pkg == "src":
        # Root src package stays as-is (the mega index.ts lives here)
        return "src"
...
def rewrite_ts_file(filepath: str, i: int) -> str:
    """Rewrite a source file path for copy i.

    src/index.ts       → src/entry_00.ts
    src/models/base.ts → src/models_00/base.ts
    """
    s = sfx(i)

    if filepath == "src/index.ts":
        return f"src/entry{s}.ts"
...
def generate_project(n: int) -> int:
    """Generate the scaled project directory. Returns number of .ts files created."""
    ...
    for i in range(n):
        ...
        # Copy src/index.ts as src/entry_XX.ts (remove trailing main() call)
        content = (SOURCE_PROJECT / "src" / "index.ts").read_text()
        content = rewrite_imports(content, i)
        # Remove the standalone main() invocation at module level
        # (the mega index.ts will call it instead)
        content = content.replace("\nmain();\n", "\n")
        (src_dir / f"entry{s}.ts").write_text(content)

    # Mega src/index.ts
    lines = []
    for i in range(n):
        lines.append(f'import {{ main as main{sfx(i)} }} from "./entry{sfx(i)}";')
    lines += ["", "export function main(): void {"]
    for i in range(n):
        lines.append(f"    main{sfx(i)}();")
    lines += ["}", "", "main();", ""]
    (src_dir / "index.ts").write_text("\n".join(lines))

    # Copy tsconfig.json and package.json
    shutil.copy2(SOURCE_PROJECT / "tsconfig.json", TARGET_PROJECT / "tsconfig.json")
    shutil.copy2(SOURCE_PROJECT / "package.json", TARGET_PROJECT / "package.json")

    # Copy node_modules (needed by TS language server)
    src_nm = SOURCE_PROJECT / "node_modules"
    if src_nm.exists():
        shutil.copytree(src_nm, TARGET_PROJECT / "node_modules", symlinks=True)
    ...
    file_count = sum(1 for _ in (TARGET_PROJECT / "src").rglob("*.ts"))
    return file_count
...
def generate_fixture(n: int) -> dict:
    ...
    fixture: dict = {"language": "TypeScript"}

    # --- References ---
    refs = []
    for i in range(n):
        for r in original["sample_references"]:
            refs.append(rewrite_ts_name(r, i))
    # Add the mega main
    ...
    for i in range(n):
        for c in original["sample_classes"]:
            classes.append(rewrite_ts_name(c, i))
    ...
    for i in range(n):
        for cls_name, exp in original.get("sample_hierarchy", {}).items():
            ...
            for key in ("superclasses_contain", "subclasses_contain"):
                if key in exp:
                    new_exp[key] = [rewrite_ts_name(s, i) for s in exp[key]]
            hierarchy[rewrite_ts_name(cls_name, i)] = new_exp
    ...
    for i in range(n):
        for src, dst in original["sample_edges"]:
            edges.append([rewrite_ts_name(src, i), rewrite_ts_name(dst, i)])
    ...
    for i in range(n):
        for pkg_name, exp in original.get("sample_package_deps", {}).items():
            new_pkg = rewrite_ts_pkg(pkg_name, i)
            if new_pkg == "src":
                # Skip — we handle the mega src package separately below
                continue
            new_exp: dict = {}
            if "imports_contain" in exp:
                new_exp["imports_contain"] = [rewrite_ts_pkg(p, i) for p in exp["imports_contain"]]
            if "imported_by_contain" in exp:
                new_exp["imported_by_contain"] = [rewrite_ts_pkg(p, i) for p in exp["imported_by_contain"]]
...
def main():
    ...
    print(f"  TypeScript files: {file_count}")

    stats = generate_fixture(n)
    print(f"Fixture: {FIXTURE_DIR / 'typescript_scaled.json'}")
