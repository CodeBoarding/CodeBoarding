...
def test_worktree_diff_includes_untracked_files():
    ...
    with patch(
        ...
        parsed = get_parsed_git_diff(Path("/tmp/repo"), "HEAD", target_ref="")
...
def test_rename_keeps_entry_when_either_side_is_source():
    ...
    with patch("repo_utils.parsed_diff.subprocess.run", side_effect=[_completed(diff_out), _completed("")]):
        parsed = get_parsed_git_diff(Path("/tmp/repo"), "HEAD", target_ref="")
...
def test_untracked_files_skipped_when_comparing_commits():
    ...
    with patch("repo_utils.parsed_diff.subprocess.run", side_effect=[_completed(diff_out)]) as run:
        parsed = get_parsed_git_diff(Path("/tmp/repo"), "v1.0", target_ref="v1.1")
