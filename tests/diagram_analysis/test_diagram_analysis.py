...
def test_restore_untouched_relations_refreshes_descendant_endpoints() -> None:
    baseline_touched = Relation(src_name="Child", dst_name="Other", relation="old", src_id="1.1", dst_id="2")
    baseline_untouched = Relation(src_name="Other", dst_name="Third", relation="stable", src_id="2", dst_id="3")
    refreshed_touched = Relation(src_name="Child", dst_name="Other", relation="new", src_id="1.1", dst_id="2")
    root = AnalysisInsights(
        description="root",
        components=[],
        components_relations=[refreshed_touched],
    )

    _restore_untouched_relations(root, [baseline_touched, baseline_untouched], {"1"})

    assert root.components_relations == [baseline_untouched, refreshed_touched]
...
class TestDiagramGenerator(unittest.TestCase):
    ...
    def test_incremental_refresh_updates_existing_parent_scope(
        ...
        gen.static_analysis.available_program_graphs.return_value = {}
    ...
    def test_finalize_and_save_skips_side_artifacts_for_partial(self, mock_save):
        ...
        gen.finalize_for_save.assert_called_once_with(
            analysis,
            {},
            preserve_unchanged_relations=False,
