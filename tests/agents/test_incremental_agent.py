...
class TestUpdateScope(unittest.TestCase):
    ...
    def test_delete_component_keeps_live_cfg_methods(self) -> None:
        ...
        agent = self._agent()
        cfg = MagicMock()
        cfg.symbol_nodes.return_value = [MagicMock(id="1.method"), MagicMock(id="2.method")]
        agent.static_analysis.get_languages = MagicMock(return_value=["python"])  # type: ignore[method-assign]
        agent.static_analysis.get_program_graph = MagicMock(return_value=cfg)  # type: ignore[method-assign]
...
class TestIncrementalRelations(unittest.TestCase):
    ...
    def test_uses_api_surface_relation_pipeline_and_attaches_static_call_sites(self) -> None:
        source = ProgramNode("pkg.api.run", ProgramNodeKind.SYMBOL, "python", "run", "api.py", NodeType.FUNCTION, 1, 5)
        target = ProgramNode(
            "pkg.worker.load", ProgramNodeKind.SYMBOL, "python", "load", "worker.py", NodeType.FUNCTION, 10, 15
        )
