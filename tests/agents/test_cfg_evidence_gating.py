...
class TestEdgeTouchesChange(unittest.TestCase):
    def test_a_changed_source_makes_the_edge_evidence(self):
        self.assertTrue(edge_touches_change(edge("pkg.caller", "pkg.callee"), {"pkg.caller"}))
    ...
    def test_a_changed_target_also_makes_the_edge_evidence(self):
        ...
        self.assertTrue(edge_touches_change(edge("pkg.caller", "pkg.callee"), {"pkg.callee"}))

    def test_an_untouched_edge_is_not_evidence(self):
        self.assertFalse(edge_touches_change(edge("pkg.caller", "pkg.callee"), {"pkg.elsewhere"}))

    def test_a_full_run_has_no_changed_members(self):
        self.assertFalse(edge_touches_change(edge("pkg.caller", "pkg.callee"), None))
        self.assertFalse(edge_touches_change(edge("pkg.caller", "pkg.callee"), set()))
